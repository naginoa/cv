...
for t in test_pic:
    ...
    for c in colors:
        ...
        g3 = (img_test[:, :, 0] - img_real[:, :, 0])
        b3 = (img_test[:, :, 1] - img_real[:, :, 1])
        r3 = (img_test[:, :, 2] - img_real[:, :, 2])
        rmean = (img_test[:, :, 2] + img_real[:, :, 2]) / 2

        dist = np.sqrt(np.sum(np.multiply(np.sum(2+rmean/256),np.sum(np.multiply(r3, r3)))) + 4*np.sum(np.multiply(g3, g3))
                        +np.sum(np.multiply((2 + (255 - rmean)/256), np.multiply(b3, b3))))
